...
const Crud = (props) => {
  ...
  let enviarCompra = () => {
    ...
    if (!enviando) {
      const resultado = window.confirm('¿Desea ENVIAR la compra?');
      if (resultado) {
        setEnviando(true);

        servicio.enviarCompra({ idEnvio: valoresFormulario.idEnvio }).then(({ data, status }) => {
          listar();
          Utileria.notifications("Compra enviada correctamente.", "SUCCESS");
          setVista(LISTADO);
          setValoresFormulario(inicial);
        }).catch((e) => {
          Utileria.errorhttp(e);
  ...
  let entregarCompra = () => {
    ...
    if (!enviando) {
      const resultado = window.confirm('¿Desea ENTREGAR la compra?');
      if (resultado) {
        setEnviando(true);

        servicio.entregarCompra({ idEnvio: valoresFormulario.idEnvio }).then(({ data, status }) => {
          listar();
          Utileria.notifications("Compra entregada correctamente.", "SUCCESS");
          setVista(LISTADO);
          setValoresFormulario(inicial);
        }).catch((e) => {
          Utileria.errorhttp(e);
  ...
  const formularioVisualizacion = () => {
    ...
    const { compra_producto, personaResponsable, status, fechaEnvio, fechaEntrega } = valoresFormulario;
    const { persona, fechaCompra, total, montoPagado, idCompra } = valoresFormulario?.compra;
      ...
      <div>
        ...
        <Row className='mb-2'>
          <Col md={9}>
            ...
            <hr />
            <h5><strong>Estatus: </strong>{status?.nombre}</h5>
            {status?.nombre === constantes.ESTATUS_PENDIENTE ? '' : <div>
              <h5><strong>Responsable: </strong> {`${personaResponsable?.nombre} ${personaResponsable?.primerApellido} ${personaResponsable?.segundoApellido ? ' ' + personaResponsable?.segundoApellido : ''}`}</h5>

              <h5><strong>Fecha de envio: </strong>{Utileria.formatDateTime(fechaEnvio)}</h5>
              {status?.nombre === constantes.ESTATUS_ENTREGADO ?
                <h5><strong>Fecha de entrega: </strong>{Utileria.formatDateTime(fechaEntrega)}</h5>
                : ''}
          ...
          <Col md={3}>
            {
              constantes.ESTATUS_PENDIENTE === status?.nombre ? (<Button onClick={() => enviarCompra()} className="my-2 border-0 btn-transition" color="info" disabled={enviando}>
                <i className="pe-7s-paper-plane" style={{ fontSize: 21 }} /> ENVIAR
              </Button>) : ''
            }
            {
              constantes.ESTATUS_EN_CAMINO === status?.nombre ?
                (<Button onClick={() => entregarCompra()} className="my-2 me-2 border-0 btn-transition mx-lg-4" color="success" disabled={enviando}>
                  <i className="pe-7s-map-marker" style={{ fontSize: 21 }} /> ENTREGADO
                </Button>) : ''
            }
          </Col>
        </Row>
